 ...
 * @returns {{x, y, z}|*}
 */
function getDirection(last) {
    var tmp = ENUM_DIRECTION [Math.floor(Math.random()*10)%3];

    // cant get the opposite of last direction.
    // ex: (1,0,0) vs (-1,0,0)
    if(last && ((last.x*tmp.x === -1) || (last.y*tmp.y === -1) || (last.z*tmp.z === -1)))
        return getDirection(last);
    return tmp;
}
